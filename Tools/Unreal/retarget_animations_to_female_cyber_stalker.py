...
TARGET_MESH_PATH = "/Game/NocturneSignal/Characters/FemaleCyberStalker/SK_FemaleCyberStalker.SK_FemaleCyberStalker"
RETARGETING_ROOT = "/Game/NocturneSignal/Characters/FemaleCyberStalker/Retargeting"
OUTPUT_ROOT = "/Game/NocturneSignal/Characters/FemaleCyberStalker/RetargetedAnimations"
...
SOURCE_SETS = [
    ...
    },
    {
        ...
        "mesh": "/Game/NocturneSignal/AnimationSources/FightingAnimations/Animations/Cross_Punch_Anim.Cross_Punch_Anim",
        "mesh_candidates": [
            "/Game/NocturneSignal/AnimationSources/FightingAnimations/SK_FightingAnimations.SK_FightingAnimations",
            "/Game/NocturneSignal/AnimationSources/FightingAnimations/Animations/Cross_Punch_Anim.Cross_Punch_Anim",
        ],
        "root": "/Game/NocturneSignal/AnimationSources/FightingAnimations/Animations",
        "output": OUTPUT_ROOT + "/FightingAnimations",
    },
]


def log(message):
    unreal.log("[NocturneFemaleCyberStalkerRetarget] " + str(message))
...
def retarget_set(source_set, target_rig, target_mesh, limit=0):
    ...
    retargeter = get_or_create_retargeter(
        "RTG_" + source_set["name"] + "_To_FemaleCyberStalker",
        source_rig,
        source_mesh,
        target_rig,
        target_mesh,
    ...
    anim_name_filter = {
        item.strip().lower()
        for item in os.environ.get("NOCTURNE_FCS_RETARGET_ANIM_NAMES", "").split(",")
        if item.strip()
    }
    if anim_name_filter:
        anim_assets = [
...
def main():
    ...
    target_rig = get_or_create_ik_rig("IK_FemaleCyberStalker", target_mesh)

    limit_text = os.environ.get("NOCTURNE_FCS_RETARGET_LIMIT", "0").strip()
    limit = int(limit_text) if limit_text else 0
    source_filter = {
        item.strip().lower()
        for item in os.environ.get("NOCTURNE_FCS_RETARGET_SOURCES", "").split(",")
        if item.strip()
    }
    ...
    try:
        ...
        command_line = ""
    if "QuitAfterFemaleCyberStalkerRetarget" in command_line:
